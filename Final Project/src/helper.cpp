...
/*                             auxiliary functions                         */
/* ----------------------------------------------------------------------- */
int LBM::min( int a, int b)           
{
    if( a < b ) return a;
    return b;
}

int LBM::max( int a, int b)
{
    if( a > b ) return a;
    return b;
}

double LBM::fmin( double a, double b)
{
    if( a < b ) return a;
    return b;
}

double LBM::fmax( double a, double b)
{
    if( a > b ) return a;
    return b;
}
...
void LBM::errhandler( int nLine, const char *szFile, const char *szString )
{
    int err = errno;
...
void LBM::read_string( const char* szFileName, const char* szVarName, char*   pVariable)
{
...
void LBM::read_int( const char* szFileName, const char* szVarName, int* pVariable)
{
...
void LBM::read_double( const char* szFileName, const char* szVarName, double* pVariable)
{
...
void LBM::write_matrix( const char* szFileName,       /* filename */
		   double **m,		       /* matrix */
...
void LBM::read_matrix( const char* szFileName,       /* filename */
		   double **m,		       /* matrix */
		   int nrl,		       /* first column */
		   int nrh,		       /* last column */
		   int ncl,		       /* first row */
...
double **LBM::matrix( int nrl, int nrh, int ncl, int nch )
{
   int i;
...
void LBM::free_matrix( double **m, int nrl, int nrh, int ncl, int nch )
{
   double **pArray  = m + nrl;
...
void LBM::init_matrix( double **m, int nrl, int nrh, int ncl, int nch, double a)
{
   int i,j;
   for( i = nrl; i <= nrh; i++)
       for( j = ncl; j <= nch; j++)
...
/* allocates storage for a matrix */
int **LBM::imatrix( int nrl, int nrh, int ncl, int nch )
{
   int i;
...
/* deallocates the storage of a matrix  */
void LBM::free_imatrix( int **m, int nrl, int nrh, int ncl, int nch )
{
   int **pArray  = m + nrl;
...
void LBM::init_imatrix( int **m, int nrl, int nrh, int ncl, int nch, int a)
{
   int i,j;
   for( i = nrl; i <= nrh; i++)
       for( j = ncl; j <= nch; j++)
...
int **LBM::read_pgm(const char *filename)
{
    FILE *input = NULL;
    char line[1024];
    int levels;
